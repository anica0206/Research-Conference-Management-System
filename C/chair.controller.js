                ...
                if (rows.length > 0) {
                    ...
                    pool.getConnection((err, conn) => {
                        ...
                        if (paramStatus == "accept") {
                            const exec = conn.query(
                                "update db.paper set status = 'Accept' where aId = ?",
                                [paramAid],
                                (err, result) => {
                                    conn.release();
                                    console.log("sql worked" + exec.sql);
                            ...
                            const exec = conn.query(
                                "update db.paper set status = 'Reject' where aId = ?",
                                [paramAid],
                                (err, result) => {
                                    conn.release();
                                    console.log("sql worked" + exec.sql);
...
module.exports = {
    viewBid, allocateBidAuto, allocateBidManual, viewReviewRating, updatePaperStatus, viewPaperList, viewAcceptPaper
    , searchBids
};